        ...
        for mode in range(24):
            ...
            for pos in rotated_scanner:
                ...
                if len(set(shifted_scanner) & set(scanner1)) >= min_count:
                    return True, shifted_scanner

    return False, scanner2


def beacons(data: List[str]) -> int:
    scanners_data = parse_data(data)
    true_scanners = {0}

    while len(true_scanners) < len(scanners_data):
        for s2, scanner2 in enumerate(scanners_data):
            if s2 in true_scanners:
                continue
            for s1 in true_scanners:
                status, scanner2 = find_overlap(scanners_data[s1], scanner2)
