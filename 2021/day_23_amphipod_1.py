...
def optimal_path_score(data: List[str]) -> int:
    ...
    while len(states_dict):
        ...
        for j, symbol in enumerate(table[1]):
            if symbol.isalpha():
                i0, j0 = CANDIDATES[symbol][0]
                room_0 = table[i0][j0]

                i1, j1 = CANDIDATES[symbol][1]
                room_1 = table[i1][j1]
                ...
                if room_0 == '.':
                    ...
                    if new_score != -1:
                        ...
                        flag = True
                elif room_0 == symbol and room_1 == '.':
                    new_score, new_table = check_path(table, (1, j), (i1, j1))
                    if new_score != -1:
                        table = new_table
                        score = new_score + score
                        ready += ENERGY[new_table[i1][j1]]
                        flag = True
        ...
        for i in range(2, 4):
            for j, symbol in enumerate(table[i]):
                if symbol.isalpha():
                    if table[i - 1][j] != '.':
                        continue
                    if (i, j) == CANDIDATES[symbol][0]:
                        continue
                    if (i, j) == CANDIDATES[symbol][1]:
                        i0, j0 = CANDIDATES[symbol][0]
...
class TestClass():
    ...
    def test_1(self):
        ...
        assert optimal_path_score(data) == 12521
