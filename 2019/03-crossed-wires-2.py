        ...
        for delta in path:
            ...
            while not np.array_equal(current, seg_start + delta):
                ...
                if m[tuple(current)] == 0:
                    ...
                    intersections.add(tuple(current))
    ...
    for path in wires:
        ...
        for delta in path:
            ...
            while not np.array_equal(current, seg_start + delta):
                ...
                if tuple(current) in intersections:
                    if tuple(current) not in distances:
                        distances[tuple(current)] = current_len
                    else:
                        distances[tuple(current)] += current_len

    min_distance = min([v for k, v in distances.items()])
