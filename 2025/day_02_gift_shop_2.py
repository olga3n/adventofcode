...
def is_invalid(value: int) -> bool:
    ...
    for size in range(1, len(str_value) // 2 + 1):
        if len(str_value) % size == 0:
            sym_value = str_value[:size] * (len(str_value) // size)
            if sym_value == str_value:
                return True
...
def test_zero_turns():
    ...
    assert 4174379265 == invalid_ids(line)
