...
def fallen_blocks(data: Iterable[str]) -> int:
    ...
    for block in blocks:
        excluded = {block.id}
        while True:
            size_exluded = len(excluded)
            for block_id, block_deps in deps.items():
                if len(block_deps) == 0:
                    continue
                if len(block_deps.intersection(excluded)) == len(block_deps):
                    excluded.add(block_id)
            if len(excluded) == size_exluded:
                break

        result += len(excluded) - 1

    return result


def test_fallen_blocks():
    data = [
        '1,0,1~1,2,1',
        '0,0,2~2,0,2',
        '0,2,3~2,2,3',
