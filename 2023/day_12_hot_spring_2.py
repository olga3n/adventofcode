...
def line_score(line: str, copies=5) -> int:
    symbols, values = line.split()
    symbols = '?'.join([symbols] * copies)
    values = ','.join([values] * copies)
...
def test_line_score():
    ...
    assert line_score(data[1]) == 16384
    assert line_score(data[2]) == 1
    assert line_score(data[3]) == 16
    assert line_score(data[4]) == 2500
    assert line_score(data[5]) == 506250
...
def test_total_score():
    ...
    assert total_score(data) == 525152
