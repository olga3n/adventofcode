...
def sum_lengths(data: List[str], extra_lines=2) -> int:
...
def test_sum_lengths():
    ...
    assert sum_lengths(data, extra_lines=10) == 1030
    assert sum_lengths(data, extra_lines=100) == 8410


def main():
    data = (line.rstrip() for line in sys.stdin)
    data = [line for line in data if len(line)]
    result = sum_lengths(data, extra_lines=1000000)
    print(result)
