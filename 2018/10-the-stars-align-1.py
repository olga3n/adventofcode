...
def build_message(points, xn, yn, min_x, min_y):
    m = np.zeros((xn, yn))

    for ind, item in enumerate(points):
        m[int(item[0] - min_x)][int(item[1] - min_y)] = 1
...
def message(data):
    ...
    return build_message(
        last_points, last_xn, last_yn, last_min_x, last_min_y)
...
class TestMethods(unittest.TestCase):
    ...
    def test_0(self):
        ...
        data_out = [x for x in data_out if len(x)]

        self.assertEqual(message(data_in), data_out)
